			...
			if tasks := jr.TaskStatusIndex[api.Pending]; len(tasks) > 0 {
				...
				for _, task := range tasks {
					...
					if task.InitResreq.IsEmpty() {
						allocateBestEffortPod(ssn, task)
					} else {
...
func allocateBestEffortPod(ssn *framework.Session, task *api.TaskInfo) {
	nodes := util.GetNodeList(ssn.Nodes)

	candidateNodes := util.PredicateNodes(task, nodes, ssn.PredicateFn)
	...
	nodeScores := util.PrioritizeNodes(task, candidateNodes, ssn.NodeOrderMapFn, ssn.NodeOrderReduceFn)
...
func selectHost(ssn *framework.Session, nodeToVictimMap map[*api.NodeInfo]*victimsOfBackFill, tr *api.TaskInfo) *api.NodeInfo {
	...
	nodeScores := util.PrioritizeNodes(tr, nodes, ssn.NodeOrderMapFn, ssn.NodeOrderReduceFn)
